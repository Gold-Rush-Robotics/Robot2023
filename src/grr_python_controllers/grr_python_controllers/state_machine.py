...
class StateMachine(Node):
    def __init__(self):
        ...
        self.action_tree.setNext(
            ...
            ServoAction(JointState(name=['small_package_sweeper_joint', 'bridge_latch_joint', 'mechanism_package_joint', 'mechanism_thruster_joint', 'mechanism_lift_joint'], position=[0.0, 100.0, 10.0, 0.0, 100.0]), .25, name="Start Sweep")
        ).setNext(
            ...
            ServoAction(JointState(name=['small_package_sweeper_joint', 'bridge_latch_joint', 'mechanism_package_joint', 'mechanism_thruster_joint', 'mechanism_lift_joint'], position=[0.0, 100.0, 100.0, 0.0, 100.0]), .5, name="Cube Pickup Positions")
        ).setNext(
            DriveToPose(Pose(position=Point(y=-0.015)), name="into cubes")
        ).setNext(
